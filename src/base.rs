...
impl Base {
    ...
    /// Activates a given inactive `Event` with multi-threaded sharing.
    ///
    /// Control of the event via the `Event` handle is shared between the
    /// closure `F` as well as the returned `Event`, which internally uses an
    /// `Arc<Mutex>`. However, since the `Base` itself can still modify the raw
    /// `event` data outside of the crate's APIs, feature `threading` must be
    /// additionally enabled. This may change as deeper analysis is done on
    /// what locks must exist and which ones are redundant.
    #[cfg(feature = "threading")]
    pub fn spawn_sync<T: Exec<SyncWeak<T>, F>, F>(
        &mut self,
        ev: Event<Inactive<T>>,
        cb: F,
    ) -> io::Result<Event<Sync<T>>> {
        // First allocate the event with no context, then apply the reference
        // to the closure (and itself) later on.
        let raw_ev = self
            .new_event_raw::<SyncWeak<T>, T, F>(&ev)
            .ok_or_else(|| io::Error::new(io::ErrorKind::Other, "Failed to allocate event"))?;

        let event: Event<Sync<T>> =
            EventInner::from_raw(raw_ev, Some(finalize_wrapped_callback::<SyncWeak<T>, T, F>))
                .into();
        let closure_event = event.downgrade();
