...
/// The primitive event-type which is created using a non-negative `RawFd`.
#[derive(Debug)]
pub struct Fd;
...
/// Multi-threaded synchronization type used by `Base::spawn_sync`.
#[derive(Clone, Debug)]
#[doc(hidden)]
pub struct Sync<T>(pub(crate) Arc<Mutex<EventInner<T>>>);
...
/// Downgraded version of `Sync` which does not count toward ownership.
#[derive(Debug)]
#[doc(hidden)]
pub struct SyncWeak<T>(pub(crate) std::sync::Weak<Mutex<EventInner<T>>>);
...
impl<T> Downgrade for Sync<T> {
    type Weak = SyncWeak<T>;

    fn downgrade(&self) -> Self::Weak {
        SyncWeak(Arc::downgrade(&self.0))
    }
}
...
impl<T> From<EventInner<T>> for Event<Sync<T>> {
    fn from(inner: EventInner<T>) -> Self {
        Event {
            inner: Sync(Arc::new(Mutex::new(inner))),
            in_callback: Arc::new(AtomicBool::new(false)),
            stopped: Arc::new(AtomicBool::new(false)),
        }
    }
...
impl<T> Event<Sync<T>> {
    ...
    where
        F: Fn(&mut EventInner<T>) -> O,
    {
        let mut ev = self.inner.0.lock().unwrap();
        f(&mut *ev)
    }
...
impl<T> Event<SyncWeak<T>> {
    fn with_inner<F, O>(&self, f: F) -> O
    where
        F: Fn(&mut EventInner<T>) -> O,
    {
        let upgraded = self.inner.0.upgrade().unwrap();
        let mut ev = upgraded.lock().unwrap();
        f(&mut *ev)
    }
